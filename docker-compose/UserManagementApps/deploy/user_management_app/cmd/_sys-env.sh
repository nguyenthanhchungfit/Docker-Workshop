...
if [ "x$JAVA_NAME" = "x" ]; then
	JAVA_VERSION=`ls /zserver/java | grep jdk1.8.0 | grep x64 | grep -v "tar" | grep -v "zip"| sed 's/jdk1.8.0_//g'| sed 's/-x64//g'|sort -n|tail -1`
	JAVA_NAME="jdk1.8.0_${JAVA_VERSION}-x64"
	if [ "x$JAVA_NAME" = "x" ]; then
...
grabMyIpAddress() {
    SYS_IP_ADDR=`/sbin/ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1' | head -1`
}
grabMyIpAddress
